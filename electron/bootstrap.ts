...
class UpdateService extends EventEmitter {
  constructor(appVersion, autoUpdater) {
    super();
...
async function createUpdateService(appVersion) {
  const { autoUpdater } = require("electron-updater");
  return new UpdateService(appVersion, autoUpdater);
}
...
const appRootDir = resolveAppRootFromModuleDir(moduleDir);
