  ...
  requestPolicyDraft = {
    ...
    allowedProviders: [
      ...
      el("requests-provider-soundcloud") instanceof HTMLInputElement && el("requests-provider-soundcloud").checked
        ? "soundcloud"
        : "",
      el("requests-provider-spotify") instanceof HTMLInputElement && el("requests-provider-spotify").checked
        ? "spotify"
        : "",
      el("requests-provider-suno") instanceof HTMLInputElement && el("requests-provider-suno").checked
        ? "suno"
        : ""
    ].filter(Boolean),
...
function renderAutoDjStatus() {
  ...
  if (connectionPill) {
    connectionPill.className = responding
      ? "status-pill status-pill--ok"
      : recentlySeen
        ? "status-pill status-pill--warn"
        : "status-pill status-pill--error";
    connectionPill.textContent = responding
      ? "Connected"
      : recentlySeen
        ? "Recently seen"
        : status?.configured
  ...
  if (activationPill) {
    activationPill.className = activation.synchronized
      ? activation.effective
        ? "status-pill status-pill--ok"
        : "status-pill status-pill--idle"
      : "status-pill status-pill--warn";
    activationPill.textContent = activation.synchronized
      ? activation.effective
        ? "Enabled"
        : "Disabled"
      : activation.desired
  ...
  setText(
    ...
    status?.takeover?.active
      ? `Viewer-request takeover active${status.takeover.expiresAt ? ` until ${new Date(status.takeover.expiresAt).toLocaleTimeString()}` : ""}.`
      : "Viewer-request takeover inactive."
  );
...
async function discoverAutoDj() {
  ...
  try {
    ...
    const serviceUrl = peer.serviceUrl || peer.url || (
      peer.address && peer.servicePort ? `http://${peer.address}:${peer.servicePort}` : ""
    );
...
function applySettingsPayload() {
  ...
  if (el("autodj-service-token") instanceof HTMLInputElement) {
    el("autodj-service-token").placeholder = settingsPayload.settings.hasAutoDjServiceToken
      ? "Saved token hidden"
      : "";
  }
...
root.addEventListener("change", async (event) => {
  ...
  if (
    (
      target.id === "requests-provider-youtube" ||
      target.id === "requests-provider-soundcloud" ||
      target.id === "requests-provider-spotify" ||
      target.id === "requests-provider-suno"
    ) &&
